#!/usr/bin/env python3
"""
Quick test of the fixed profile-based collection.
Runs 5 scrolls and reports what we get.
"""
import json
import subprocess
import sys
...
def test_profile_collection():
    print("=== Testing fixed profile collection ===")
    
    # Navigate to profile
    print("Navigating to x.com/elonmusk...")
    ...
    ws_dt = dt2(2026, 4, 14, 0, 0, 0, tzinfo=timezone.utc)
    we_dt = dt2(2026, 4, 21, 23, 59, 59, tzinfo=timezone.utc)
    ...
    for i in range(5):
        ...
        for raw_t in raw:
            ...
            tweets.append(p)
            new += 1
        print(f"  Scroll {i+1}: +{new} new (total in window: {len(tweets)})")
    
    print(f"\nTotal tweets in Apr14-21 window: {len(tweets)}")

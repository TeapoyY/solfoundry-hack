...
export function BountyGrid() {
  ...
  // Client-side search filter across title, description, and skills/category
  const filteredBounties = useMemo(() => {
    if (!debouncedQuery) return allBounties;
    const q = debouncedQuery.toLowerCase();
    return allBounties.filter((bounty) => {
      const titleMatch = bounty.title.toLowerCase().includes(q);
      const descMatch = bounty.description?.toLowerCase().includes(q);
      const skillMatch = bounty.skills?.some((s) => s.toLowerCase().includes(q));
      const categoryMatch = bounty.category?.toLowerCase().includes(q);
      return titleMatch || descMatch || skillMatch || categoryMatch;
    });
